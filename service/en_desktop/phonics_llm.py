...
2. 所有片段的 letters 依次拼接必须逐字符等于单词本身（大小写不敏感），不能多字母也不能少字母
3. 所有片段的 ipa 依次拼接必须逐字符等于给定音标去掉 / ˈ ˌ . 和空格后的结果——每个音素只能出现
   在一个片段里，绝不能把同一个音素在两个相邻片段里各写一次，也不能漏掉音标里的任何音素
4. 音标里的字符要原样使用（比如音标写的是 ɪ 就必须用 ɪ，不能自己换成看起来像的 i；音标写的是
   eɪ 就必须整体用 eɪ，不能拆成 e 和 ɪ 两段）
5. 只返回 JSON，不要任何解释文字，格式：{{"segments": [{{"letters": "...", "ipa": "..."}}]}}
